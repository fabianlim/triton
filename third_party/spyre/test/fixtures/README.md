...
| `factory` | `VariantFactory` | Optional. Supplies the fields that depend on the swept `params` combination, so one declaration spans e.g. `OP` × `DTYPE` instead of being copied per point. Subclass `VariantFactory` (in `test/conftest.py`) and define the hooks you need — `signature()` → the variant `SIGNATURE`, `reference()` → `reference`, `inputs()` → `inputs`. Each is called once per combination with the combination as keyword arguments (plain values, not the `(label, value)` pairs used to build keys, so declare `**_` for the axes you ignore) and returns that field's value, or `None` to leave it unset. Declaring a hook *and* the literal field it produces on the same merged variant raises at collection time — which one won would otherwise depend on statement order. `base` needs nothing special: `factory` is a single value, replaced wholesale; use `dataclasses.replace(...)` to inherit one with a field changed. `extra_checks` is deliberately **not** a hook (see #71). Pinned by `test/python/factory-protocol-test.py`. |
