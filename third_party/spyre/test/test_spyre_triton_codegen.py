...
# ---------------------------------------------------------------------------
# Stub triton compilation and kernel execution.
#
# All kernels generated by SpyreTritonScheduling use raw tt.load/tt.store ops
# (Inductor's standard pointer-based codegen) — the Spyre KTIR passes require
# descriptor-based ops (tt.make_tensor_descriptor etc.) which are only emitted
# by SpyreTritonKernel for FixedTiledLayout buffers. Those layouts require real
# Spyre tensor metadata not available on CPU.
#
# KTIR capture therefore produces nothing for the current test ops. The Triton
# source emission path is what this test exercises; structural KTIR assertions
# will be added once the ops produce descriptor-based kernels.
# ---------------------------------------------------------------------------
