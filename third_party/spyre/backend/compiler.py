...
_SPYRECODE_STAGE_PASSES = (
    ...
    # MEASUREMENT-BRANCH NOTE: with the generic layout pass selected this pass
    # already ran in _CORE_PIPELINE_PASSES (before the layout pass), so listing it
    # here would run it twice. Harmless -- the second run finds no fill -- but
    # skipped for clarity. The comment above describes the pre-aa4c685f8 gate and
    # is stale: the pass now drops the fill for every combiner.
    *(() if _LAYOUT_PASS_VARIANT == "generic" else ("drop_reduction_init_fill",)),
...
class SpyreBackend(BaseBackend):
    ...
    def parse_options(self, options: dict) -> SpyreOptions:
        ...
        #
        # Where this anchors depends on which layout pass is installed, and
