...
//
// Both maps being the identity is what "absorbed" has to look like once the loop
// domain is numbered from the result: the transpose lives in the two markers
// disagreeing about which logical dim each physical position carries, and under
// these two markers the physicalized operands stand in an identity relation.
// A permuted pair here would say the same thing in a frame the result is not
// written in -- see buildLoopDomain.
