...
#### ✅ `test_batch_matmul_dispatches`

```python
C[B,M,N] = A[B,M,K] @ B[B,K,N]; A stick-on-K (K-stick reduction loop), B stick-on-N (parallel N-stick). dispatchBatchMatmul lowers tt.dot to linalg.batch_matmul and wraps the K-stick iteration in scf.for.
```

<sup>Source: `third_party/spyre/test/test_rewrite_descriptor_layout.py:1502` (`TestBatchMatmul.test_batch_matmul_dispatches`)</sup>
...
_Patterns without round-trip evidence: `batch-matmul-stick`, `expand-dims`, `join`, `reshape`, `splat`, `split`, `transpose`. Add a tagged fixture variant to verify end-to-end._
