...
//      f. Retype identities — rebuild any identity the descriptor layout left
//                            at logical rank while the partials went physical.
//      g. Build combiner   — shorthand → linalg.fill+op; region → transcribe.
//      h. Emit delivery    — ktdp.inter_tile_reduce + yield_reduced region.
//      i. Emit dep set     — producer_dependency_per_consumer from depWkSlices.
//      j. RAUW + erase     — replace tt op uses with delivery results.
//
//===----------------------------------------------------------------------===//
